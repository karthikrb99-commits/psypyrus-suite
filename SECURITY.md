...
Security is of paramount importance to **PsyPyrus Suite** because it is a mental health operating system that facilitates the processing of Protected Health Information (PHI). If you discover a security vulnerability, please do NOT report it publicly via a GitHub issue. Instead, follow the process outlined below.

---

## Supported Versions

Only the latest release version of the PsyPyrus Suite is supported for security updates.
...
- The platform (Web, Android, iOS, macOS, Desktop) and version affected.
...
PsyPyrus operates as a **Clinical Decision Support System (CDSS)**. Any security vulnerabilities that compromise data integrity, client-side encryption (such as SQLite/CoreData/IndexedDB envelope encryption), or diagnostic engine operations will be treated with the highest severity. 
