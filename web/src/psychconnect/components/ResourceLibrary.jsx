...
export default function ResourceLibrary({ currentUser, allUsers }) {
    ...
    const [gamificationProfile, setGamificationProfile] = useState(() => 
        isPsychologist ? null : GamificationService.getProfile('Patient')
    );

    useEffect(() => {
        if (!isPsychologist) {
            setGamificationProfile(GamificationService.getProfile('Patient'));
        }
    }, [isPsychologist]);
    ...
    return (_jsxs("div", { className: "max-w-7xl mx-auto px-4 py-8 text-left", id: "resource-library-root", children: [_jsxs("div", { className: "flex flex-col md:flex-row md:items-center md:justify-between mb-8 border-b border-white/5 pb-5", children: [_jsxs("div", { children: [_jsxs("h1", { className: "text-xl font-light tracking-wide text-slate-100 uppercase flex items-center", style: { fontFamily: "Georgia, serif" }, children: [_jsx(BookOpen, { className: "w-5.5 h-5.5 text-indigo-500 mr-2 shrink-0 animate-pulse" }), "Therapeutic Resource Library"] }), _jsx("p", { className: "text-xs text-slate-400 mt-1 max-w-xl", children: "Browse and coordinate professional cognitive-behavioral tools, deep somatic breathing sheets, and verified anxiety containment worksheets." })] }), _jsxs("div", { className: "flex bg-[#0D0D0F] border border-white/5 rounded-xl p-1 mt-4 md:mt-0 max-w-fit shadow-md", children: [_jsx("button", { id: "tab-browse-resources", onClick: () => setActiveTab("browse"), className: `px-4 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer select-none uppercase tracking-wider ${activeTab === "browse" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"}`, children: "All Templates" }), _jsxs("button", { id: "tab-assignments", onClick: () => setActiveTab("assignments"), className: `px-4 py-1.5 rounded-lg text-xs font-semibold transition cursor-pointer select-none uppercase tracking-wider flex items-center ${activeTab === "assignments" ? "bg-indigo-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-200"}`, children: [_jsx("span", { children: isPsychologist ? "My Assignments" : "Assigned Homework" }), assignedList.filter(r => !r.isCompleted).length > 0 && (_jsx("span", { className: "w-2 h-2 ml-1.5 rounded-full bg-rose-500 animate-pulse" }))] })] })] }), !isPsychologist && gamificationProfile && _jsxs("div", { className: "mb-8 p-6 bg-gradient-to-r from-teal-950/20 to-indigo-950/20 border border-teal-500/10 rounded-2xl shadow-xl flex flex-col lg:flex-row gap-6 justify-between items-center text-left", children: [_jsxs("div", { className: "flex items-center gap-4 w-full lg:w-auto", children: [_jsxs("div", { className: "w-14 h-14 rounded-full bg-teal-500/10 border border-teal-500/30 flex items-center justify-center font-bold text-teal-400 text-lg shadow-inner shrink-0", children: [_jsx("span", { className: "text-slate-400 text-xs mr-0.5", children: "Lvl" }), _jsx("span", { className: "text-xl font-black text-white", children: gamificationProfile.level })] }), _jsxs("div", { className: "flex-1 min-w-0", children: [_jsxs("h2", { className: "text-sm font-bold text-slate-100 tracking-wide flex items-center gap-2", children: [_jsx("span", { children: "Somatic Study Room" }), _jsx("span", { className: "px-2 py-0.5 rounded-md text-[9px] bg-teal-500/10 text-teal-400 border border-teal-500/20 font-mono font-bold uppercase", children: "Active" })] }), _jsxs("div", { className: "flex items-center gap-2 mt-2", children: [_jsx("div", { className: "relative overflow-hidden bg-white/10 rounded-full h-2 w-48 shadow-inner", children: [_jsx("div", { className: "bg-gradient-to-r from-teal-400 to-indigo-500 h-full rounded-full transition-all duration-[660ms] ease-out", style: { width: `${Math.min(100, Math.round((gamificationProfile.xp / gamificationProfile.nextLevelXp) * 100))}%` } })] }), _jsxs("span", { className: "text-[10px] font-mono text-slate-400 font-bold", children: [gamificationProfile.xp, "/", gamificationProfile.nextLevelXp, " XP"] })] })] })] }), _jsxs("div", { className: "flex flex-wrap gap-4 w-full lg:w-auto justify-start lg:justify-end items-center", children: [_jsxs("div", { className: "bg-[#0A0A0C]/50 border border-white/5 rounded-xl px-4 py-2 text-center min-w-[90px] shadow-sm", children: [_jsx("span", { className: "block text-[9px] uppercase font-bold text-slate-500 tracking-wider font-mono", children: "MindCoins" }), _jsxs("span", { className: "text-sm font-bold text-amber-400 flex items-center justify-center gap-1 mt-0.5", children: [_jsx("span", { children: "🪙" }), gamificationProfile.coins] })] }), _jsxs("div", { className: "bg-[#0A0A0C]/50 border border-white/5 rounded-xl px-4 py-2 text-center min-w-[90px] shadow-sm", children: [_jsx("span", { className: "block text-[9px] uppercase font-bold text-slate-500 tracking-wider font-mono", children: "Completed" }), _jsxs("span", { className: "text-sm font-bold text-emerald-400 flex items-center justify-center gap-1 mt-0.5", children: [_jsx("span", { className: "fa-solid fa-circle-check text-xs mr-0.5" }), assignedList.filter(r => r.isCompleted).length] })] }), _jsxs("div", { className: "bg-[#0A0A0C]/50 border border-white/5 rounded-xl px-4 py-2 text-center min-w-[90px] shadow-sm", children: [_jsx("span", { className: "block text-[9px] uppercase font-bold text-slate-500 tracking-wider font-mono", children: "Pending" }), _jsxs("span", { className: "text-sm font-bold text-indigo-400 flex items-center justify-center gap-1 mt-0.5", children: [_jsx("span", { className: "fa-solid fa-hourglass-half text-xs mr-0.5 animate-pulse" }), assignedList.filter(r => !r.isCompleted).length] })] }), (() => { const hwQuest = gamificationProfile.quests?.find(q => q.type === 'COMPLETE_HOMEWORK'); if (!hwQuest) return null; return _jsxs("div", { className: `border rounded-xl px-4 py-2 min-w-[145px] flex items-center gap-2.5 shadow-sm ${hwQuest.completed ? 'bg-emerald-950/10 border-emerald-500/20 text-emerald-400' : 'bg-[#0A0A0C]/50 border-white/5 text-slate-400'}`, children: [_jsx("i", { className: `fa-solid ${hwQuest.completed ? 'fa-award text-emerald-400 text-lg animate-bounce' : 'fa-trophy text-amber-400 text-sm'}` }), _jsxs("div", { className: "text-left", children: [_jsx("span", { className: "block text-[8px] uppercase font-bold tracking-wider font-mono text-slate-500", children: "Active Quest" }), _jsx("span", { className: "text-[10px] font-bold block truncate max-w-[90px] text-slate-200", children: hwQuest.title }), _jsxs("span", { className: "text-[9px] font-mono font-semibold block text-slate-400", children: [hwQuest.completed ? "COMPLETED" : `Progress: ${hwQuest.current}/${hwQuest.target}`] })] })] }, "active-quest-widget"); })()] })] }), activeTab === "browse" ? (_jsxs("div", { className: "space-y-6", id: "browse-templates-screen", children: [_jsxs("div", { className: "bg-[#0D0D0F] border border-white/5 p-4 rounded-xl flex flex-wrap gap-4 items-center justify-between shadow-sm", children: [_jsxs("div", { className: "flex flex-wrap gap-3", children: [_jsxs("div", { className: "space-y-1", children: [_jsx("span", { className: "block text-[9px] uppercase font-bold text-slate-500 tracking-wide font-mono", children: "Filter by Resource Type" }), _jsxs("select", { id: "filter-type-select", value: filterType, onChange: (e) => setFilterType(e.target.value), className: "bg-[#0A0A0C] border border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none cursor-pointer [color-scheme:dark]", children: [_jsx("option", { value: "all", children: "All Templates" }), _jsx("option", { value: "article", children: "Articles" }), _jsx("option", { value: "worksheet", children: "Worksheets" }), _jsx("option", { value: "exercise", children: "Guided Exercises" })] })] }), _jsxs("div", { className: "space-y-1", children: [_jsx("span", { className: "block text-[9px] uppercase font-bold text-slate-500 tracking-wide font-mono", children: "Filter by Topic" }), _jsxs("select", { id: "filter-topic-select", value: filterTopic, onChange: (e) => setFilterTopic(e.target.value), className: "bg-[#0A0A0C] border border-white/10 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none cursor-pointer [color-scheme:dark]", children: [_jsx("option", { value: "all", children: "All Topics" }), topics.map((t) => (_jsx("option", { value: t, children: t }, t)))] })] })] }), _jsxs("div", { className: "text-[11px] font-mono text-slate-500", children: ["Showing ", _jsx("span", { className: "text-indigo-400 font-bold", children: filteredTemplates.length }), " structured resource templates"] })] }), _jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6", id: "templates-grid", children: filteredTemplates.map((res) => {
