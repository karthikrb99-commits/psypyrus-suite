...
def get_xcode_uuid(name):
    ...
    ns = uuid.UUID('86fa3d6b-dc79-4ff8-a3f2-ef67756f7db2') # Unique namespace for macOS target
    u = uuid.uuid5(ns, name)
    return u.hex[:24].upper()

project_name = "PsyPyrus"
bundle_identifier = "com.example.PsyPyrus.mac"

# Define the file hierarchy matching our actual folder structure
# We map the files to their groups. The paths are relative to the shared ios directory source.
files_and_groups = {
...
project_uuid = get_xcode_uuid("macos_project")
target_uuid = get_xcode_uuid("macos_target")
target_product_uuid = get_xcode_uuid("macos_target_product")
sources_build_phase_uuid = get_xcode_uuid("macos_sources_build_phase")
frameworks_build_phase_uuid = get_xcode_uuid("macos_frameworks_build_phase")
resources_build_phase_uuid = get_xcode_uuid("macos_resources_build_phase")
main_group_uuid = get_xcode_uuid("macos_main_group")
products_group_uuid = get_xcode_uuid("macos_products_group")
product_ref_uuid = get_xcode_uuid("macos_product_ref")
build_config_list_proj_uuid = get_xcode_uuid("macos_build_config_list_proj")
build_config_list_targ_uuid = get_xcode_uuid("macos_build_config_list_targ")
config_debug_proj_uuid = get_xcode_uuid("macos_config_debug_proj")
config_release_proj_uuid = get_xcode_uuid("macos_config_release_proj")
config_debug_targ_uuid = get_xcode_uuid("macos_config_debug_targ")
config_release_targ_uuid = get_xcode_uuid("macos_config_release_targ")

# Unique UUID mapping for files
file_uuids = {}
build_uuids = {}
for filepath in files_and_groups:
    file_uuids[filepath] = get_xcode_uuid(f"macos_file_ref_{filepath}")
    build_uuids[filepath] = get_xcode_uuid(f"macos_build_file_{filepath}")
...
group_uuids = {g: get_xcode_uuid(f"macos_group_{g}") for g in groups}
...
for filepath in files_and_groups:
    # Set Swift source file properties - path relative to SOURCE_ROOT (psypyrus/macos)
    pbxproj += f"\t\t{file_uuids[filepath]} /* {os.path.basename(filepath)} */ = {{isa = PBXFileReference; lastKnownFileType = sourcecode.swift; name = {os.path.basename(filepath)}; path = ../ios/{project_name}/{filepath}; sourceTree = SOURCE_ROOT; }};\n"
...
print("macOS Xcode project generated successfully in PsyPyrus.xcodeproj/project.pbxproj")
